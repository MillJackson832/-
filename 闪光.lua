...
do
    ...
    characterGroup:AddSlider("WalkSpeed", { Text = "步行速度", Default = 20, Min = 20, Max = 24, Rounding = 1, Callback = function(v) currentwalkspeed = v end })
    characterGroup:AddCheckbox("NoVelocity", { Text = "无流速", Default = false, Callback = function(v) noVelocityEnabled = v toggleNoVelocity() end })
    characterGroup:AddCheckbox("BunnyHop", { Text = "兔子跳", Default = false, Callback = function(v) bunnyHopEnabled = v toggleBunnyHop() end })
    characterGroup:AddSlider("BunnyHopDelay", { Text = "兔子跳延迟", Default = 1, Min = 0, Max = 5, Rounding = 2, Suffix = "s", Callback = function(v) bunnyHopDelay = v end })
    characterGroup:AddDivider()
    characterGroup:AddCheckbox("XRay", { Text = "X-光", Default = false, Callback = function(v) 
        xrayenabled = v 
        if v then 
    ...
    characterGroup:AddSlider("XRayTransparency", { Text = "X射线透明度", Default = 60, Min = 0, Max = 100, Rounding = 1, Suffix = "%", Callback = function(v) 
    ...
    otherGroup:AddToggle("AutoRespawn", { Text = "自动重生", Default = false, Callback = function(v) 
    ...
    otherGroup:AddSlider("AutoRespawnDelay", { Text = "自动重生延迟", Default = 0, Min = 0, Max = 3, Rounding = 2, Suffix = "s", Callback = function(v) autoRespawnDelay = v end })
...
do
    ...
    esptab:AddCheckbox("ESP", { Text = "透视", Default = false, Callback = function(v) 
        espenabled = v 
        if v then 
    ...
    end }):AddColorPicker("ESPColor", { Default = Color3.fromRGB(255,255,255), Title = "ESP 颜色", Callback = function(v) espconfig.espcolor = v end })

    esptab:AddCheckbox("Outline", { Text = "大纲", Default = false, Callback = function(v) 
        outlineenabled = v 
        if v then 
            for _, p in pairs(Players:GetPlayers()) do 
                if p ~= LocalPlayer then
    ...
    end }):AddColorPicker("OutlineColor", { Default = Color3.fromRGB(255,255,255), Title = "大纲 颜色", Callback = function(v) espconfig.outlinecolor = v end })
    :AddColorPicker("OutlineFillColor", { Default = Color3.fromRGB(255,255,255), Title = "填充颜色", Callback = function(v) espconfig.outlinefillcolor = v end })

    esptab:AddCheckbox("Tracers", { Text = "示踪剂", Default = false, Callback = function(v) 
        tracersenabled = v 
        if v then
            createtracers()
    ...
    end }):AddColorPicker("TracerColor", { Default = Color3.fromRGB(255,255,255), Title = "示踪颜色", Callback = function(v) espconfig.tracercolor = v end })

    configtab:AddCheckbox("RainbowESP", { Text = "彩虹ESP", Default = false, Callback = function(v) espconfig.rainbowesp = v end })
    configtab:AddCheckbox("RainbowOutline", { Text = "彩虹轮廓", Default = false, Callback = function(v) espconfig.rainbowoutline = v end })
    configtab:AddCheckbox("RainbowTracers", { Text = "彩虹示踪剂", Default = false, Callback = function(v) espconfig.rainbowtracers = v end })
    configtab:AddSlider("ESPSize", { Text = "ESP尺寸", Default = 16, Min = 16, Max = 48, Rounding = 1, Callback = function(v) espconfig.espsize = v end })
    configtab:AddDropdown("TracerPosition", { Values = {"Bottom", "Middle", "Up"}, Default = 1, Text = "示踪剂位置", Callback = function(v) espconfig.tracerposition = v end })
    configtab:AddSlider("OutlineTransparency", { Text = "轮廓透明度", Default = 0, Min = 0, Max = 100, Rounding = 1, Suffix = "%", Callback = function(v) espconfig.outlinetransparency = v / 100 end })
    configtab:AddSlider("OutlineFillTransparency", { Text = "轮廓填充透明度", Default = 50, Min = 0, Max = 100, Rounding = 1, Suffix = "%", Callback = function(v) espconfig.outlinefilltransparency = v / 100 end })
    configtab:AddSlider("RainbowSpeed", { Text = "彩虹般的速度", Default = 5, Min = 1, Max = 10, Rounding = 1, Callback = function(v) espconfig.rainbowspeed = v end })

    local gamegroup = Tabs.Visuals:AddRightGroupbox("Game", "gamepad-2")
    gamegroup:AddSlider("FieldOfView", { Text = "视场", Default = 70, Min = 60, Max = 120, Rounding = 1, Callback = function(v) Camera.FieldOfView = v end })
    gamegroup:AddCheckbox("FullBright", { Text = "全亮", Default = false, Callback = function(v) 
        if v then 
    ...
    gamegroup:AddCheckbox("NoFog", { Text = "没有雾", Default = false, Callback = function(v) 
        if v then 
...
do
    ...
    aimlocktab:AddCheckbox("AimLock", { Text = "激活自瞄", Default = false, Callback = function(v) 
        aimlockenabled = v 
        toggleAimlock() 
    end })

    aimlocktab:AddDropdown("AimLockType", { Values = {"Nearest Player", "Nearest Mouse"}, Default = 1, Text = "自瞄类型", Callback = function(v) aimlocktype = v end })

    aimlocktab:AddToggle("AutoFire", { Text = "自动点火(W.I.P)", Default = false, Callback = function(v) 
        autoFireEnabled = v 
        toggleAutoFire() 
    end })
    ...
    aimlocktab:AddCheckbox("AimLockCertainPlayer", { Text = "自瞄 某些玩家", Default = false, Callback = function(v) aimlockcertainplayer = v end })
    aimlocktab:AddDropdown("AimLockPlayerSelect", { SpecialType = "Player", ExcludeLocalPlayer = true, Text = "选择播放器", Callback = function(v) selectedplayer = v end })

    aimlocktab:AddCheckbox("EnableFOV", { Text = "启用FoV", Default = false, Callback = function(v) 
        fovenabled = v 
        if v then Options.AimLockType.Disabled = true else Options.AimLockType.Disabled = false end 
    end }):AddColorPicker("FOVColor", { Default = Color3.fromRGB(255,255,255), Title = "FOV 颜色", Callback = function(v) fovcolor = v end })

    aimlocktab:AddCheckbox("ShowFOV", { Text = "显示FOV", Default = false, Callback = function(v) 
        showfov = v 
        if v then 
            if not fovgui then 
    ...
    aimlockconfigtab:AddSlider("NearestPlayerDistance", { Text = "最近的玩家锁定距离(螺柱)", Default = 1000, Min = 10, Max = 5000, Rounding = 1, Callback = function(v) nearestplayerdistance = v end })
    aimlockconfigtab:AddSlider("NearestMouseDistance", { Text = "最近的鼠标锁定距离(螺柱)", Default = 500, Min = 10, Max = 5000, Rounding = 1, Callback = function(v) nearestmousedistance = v end })
    aimlockconfigtab:AddSlider("FOVLockDistance", { Text = "FOV锁紧距离(双头螺栓)", Default = 1000, Min = 50, Max = 5000, Rounding = 1, Callback = function(v) fovlockdistance = v end })
    aimlockconfigtab:AddCheckbox("SmoothAimlock", { Text = "平滑 自瞄", Default = false, Callback = function(v) smoothaimlock = v end })
    aimlockconfigtab:AddSlider("SmoothAimlockSpeed", { Text = "平滑 自瞄 速度 (lerp 阿尔法)", Default = 400, Min = 100, Max = 1000, Rounding = 0, Callback = function(v) lerpalpha = v / 1000 end })
    aimlockconfigtab:AddCheckbox("IgnoreShielded", { Text = "忽略屏蔽", Default = true, Callback = function(v) ignoreShielded = v end })
    aimlockconfigtab:AddCheckbox("IgnoreLobby", { Text = "忽略大堂", Default = true, Callback = function(v) ignoreLobby = v end })
    aimlockconfigtab:AddDropdown("AimPart", { Values = {"Head", "Torso", "Feet"}, Default = 1, Text = "自瞄零件", Callback = function(v) aimpart = v end })
    aimlockconfigtab:AddCheckbox("RainbowFOV", { Text = "彩虹FOV", Default = false, Callback = function(v) rainbowfov = v end })
    aimlockconfigtab:AddSlider("FOVSize", { Text = "FOV 尺寸", Default = 100, Min = 1, Max = 750, Rounding = 1, Callback = function(v) fovsize = v if fovframe then fovframe.Size = UDim2.new(0, v, 0, v) end end })
    aimlockconfigtab:AddSlider("FOVStrokeThickness", { Text = "FOV 冲程厚度", Default = 2, Min = 1, Max = 10, Rounding = 1, Callback = function(v) fovstrokethickness = v if fovstroke then fovstroke.Thickness = v end end })
    aimlockconfigtab:AddSlider("AutoFireDelay", { Text = "自动点火延迟 (W.I.P)", Default = 1.5, Min = 1.5, Max = 3, Rounding = 2, Suffix = "s", Callback = function(v) autoFireDelay = v end })
    aimlockconfigtab:AddSlider("AutoFireShootDelay", { Text = "自动射击延时 (W.I.P)", Default = 0.1, Min = 0.1, Max = 1, Rounding = 2, Suffix = "s", Callback = function(v) autoFireShootDelay = v end })
    aimlockconfigtab:AddDivider()
    aimlockconfigtab:AddCheckbox("WallCheck", { Text = "墙体检查", Default = true, Callback = function(v) wallcheckenabled = v end })
    aimlockconfigtab:AddDivider()
    ...
    aimlockconfigtab:AddSlider("AimlockOffsetY", { Text = "自瞄 偏移 (Y)", Default = 0, Min = -1, Max = 1, Rounding = 2, Callback = function(v) aimlockOffsetY = v end })
    aimlockconfigtab:AddSlider("AimlockOffsetX", { Text = "自瞄 偏移 (X)", Default = 0, Min = -1, Max = 1, Rounding = 2, Callback = function(v) aimlockOffsetX = v end })
end
...
do 
    ...
    local function makeImpactBall(part)
        ...
        outline.Image = "rbxassetid://131604517"
    ...
    featuresGroup:AddToggle("KnifeCloseToggle", { Text = "靠近玩家时切换到刀", Default = false, Callback = function(v) knifeCloseEnabled = v toggleKnifeSwitch() end })
    featuresGroup:AddSlider("KnifeRange", { Text = "S.T.K.W.C.T.P.系列", Default = 10, Min = 1, Max = 50, Rounding = 1, Suffix = " studs", Callback = function(v) knifeRange = v updateKnifeRangeSphere() end })
    featuresGroup:AddToggle("ShowKnifeRange", { Text = "显示S.T.K.W.C.T.P.范围", Default = false, Callback = function(v) showKnifeRange = v updateKnifeRangeSphere() end }):AddColorPicker("KnifeRangeColor", { Default = Color3.fromRGB(255,255,255), Title = "范围颜色", Transparency = 0.5, Callback = function(v, t) knifeRangeColor = v knifeRangeTransparency = t or 0.5 updateKnifeRangeSphere() end })
    featuresGroup:AddDivider()
    featuresGroup:AddButton("Mass Open Knife Crate", openKnifeCrates)
    featuresGroup:AddSlider("KnifeCrateCount", { Text = "刀箱打开次数", Default = 0, Min = 0, Max = 25, Rounding = 1, Callback = function(v) knifeCrateCount = math.floor(v) end })
    featuresGroup:AddButton("Mass Open Gun Crate", openGunCrates)
    featuresGroup:AddSlider("GunCrateCount", { Text = "打开的枪箱数量", Default = 0, Min = 0, Max = 15, Rounding = 1, Callback = function(v) gunCrateCount = math.floor(v) end })
...
do
    ...
    funGroup:AddCheckbox("RGBAsync", { Text = "RGB异步", Default = false, Callback = function(v) rgbAsyncEnabled = v end })
    funGroup:AddSlider("RGBAsyncSpeed", { Text = "RGB 异步 速度", Default = 10, Min = 1, Max = 50, Rounding = 1, Callback = function(v) rgbAsyncSpeed = v end })
    funGroup:AddDropdown("RGBAsyncMode", { Values = {"Forward", "Backwards"}, Default = 2, Text = "RGB异步频谱模式", Callback = function(v) rgbAsyncMode = v end })
    funGroup:AddDivider()
    funGroup:AddToggle("RGBGunKnife", { Text = "RGB枪/刀", Default = false, Callback = function(v) rgbGunKnifeEnabled = v toggleRGBGunKnife() end })
    funGroup:AddSlider("RGBSpeed", { Text = "RGB 速度 (本地)", Default = 10, Min = 1, Max = 50, Rounding = 0, Callback = function(v) rgbSpeed = v end })
    funGroup:AddSlider("RGBReapplySpeed", { Text = "RGB重新应用速度", Default = 1, Min = 0, Max = 5, Rounding = 1, Suffix = "s", Callback = function(v) rgbReapplySpeed = v end })
    funGroup:AddDropdown("RGBType", { Values = {"Material", "Highlight"}, Default = 1, Text = "RGB 类型", Callback = function(v) rgbType = v end })
    funGroup:AddToggle("RGBForceNeon", { Text = "将材质更改为霓虹灯", Default = true, Callback = function(v) getgenv().RGB_ForceNeon = v end })
    funGroup:AddDivider()
    funGroup:AddInput("HitSFX", { Default = "", Text = "点击SFX", Placeholder = "rbxassetid://...", Callback = function(v) hitSfxId = v:gsub("rbxassetid://", "") end })
    funGroup:AddInput("CritSFX", { Default = "", Text = "关键SFX", Placeholder = "rbxassetid://...", Callback = function(v) critSfxId = v:gsub("rbxassetid://", "") end })
    funGroup:AddButton("Apply Custom SFX", applyCustomSFX)
    funGroup:AddToggle("AutoApplySFX", { Text = "自动应用自定义SFX", Default = false, Callback = function(v) autoApplySfx = v toggleAutoApplySFX() end })
    funGroup:AddSlider("AutoApplyDelay", { Text = "自动应用延迟", Default = 1, Min = 0, Max = 5, Rounding = 2, Suffix = "s", Callback = function(v) autoApplyDelay = v end })
end
...
menugroup:AddInput("DPIScale", { Default = "100", Text = "DPI量表", Callback = function(v) local dpi = tonumber(v) if dpi then Library:SetDPIScale(dpi) end end })
menugroup:AddCheckbox("KeybindMenuOpen", { Default = Library.KeybindFrame.Visible, Text = "打开KeyBind菜单", Callback = function(v) Library.KeybindFrame.Visible = v end })
menugroup:AddCheckbox("ShowCustomCursor", { Text = "自定义光标", Default = true, Callback = function(v) Library.ShowCustomCursor = v end })
menugroup:AddDropdown("NotificationSide", { Values = { "Left", "Right" }, Default = "Right", Text = "通知端", Callback = function(v) Library:SetNotifySide(v) end })
menugroup:AddDropdown("DPIDropdown", { Values = { "50%", "75%", "100%", "125%", "150%", "175%", "200%" }, Default = "100%", Text = "DPI量表", Callback = function(v) v = v:gsub("%%", "") local dpi = tonumber(v) Library:SetDPIScale(dpi) end })
menugroup:AddDivider()
menugroup:AddLabel("Menu bind"):AddKeyPicker("MenuKeybind", {Default = "U", NoUI = true, Text = "菜单键绑定"})
...
task.spawn(function()
    while true do
        ...
        local kills = ls:FindFirstChild("Kills"); if not kills then continue end
        local v = kills.Value
        ...
        if v > 50 and v <= 53 then
            ...
            Library:Notify({
                Title = "慢点!如果你短时间内杀了太多的玩家，你的账号可能会被封禁!",
                Duration = 5
            })
        end
    end
